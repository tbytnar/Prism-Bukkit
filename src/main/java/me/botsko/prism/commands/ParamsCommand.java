...
public class ParamsCommand implements SubHandler {
    ...
    private static TextComponent colourParamHelp(TextComponent message) {
        Pattern pattern = Pattern.compile("([abtrkpew]|id|since|before){1}:([\\[,<,a-z,0-9,>,|,:,\\],#]*)");
        return message.replaceFirst(pattern, builder -> builder.color(NamedTextColor.LIGHT_PURPLE));
    }
}